...
const ProductsPage = () => {
    ...
    TODO:
      ...
      Instead of modifying the data locally we want to do it server side via a post
      request
  */
    /*
    TODO:
    Webapp.
      You must develop a component to display the product name, product id 
      and the product display image for each image. You can develop this 
      component any way you wish (NOTE the current style system uses tailwind 
      it is STRONGLY advised you use this style system). Then you will use 
      this component to display all the products found in the database 
      that are active. You will implement this page 
      at `webapp/src/pages/ProductsPage/ProductsPage.tsx`.
 */
    ...
    const getProducts = async () => {
        ...
        const { productData, hasError } = await getAllProducts();
        setData(productData);
        setLoadingState(hasError ? DATA_STATES.error : DATA_STATES.loaded);
    };
    ...
    if (loadingState === DATA_STATES.waiting)
        ...
        content = (
            <>
                <label className="inline-flex justify-items-center mt-3">
                    <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={handleCheckboxChange}
                        className="form-checkbox h-8 w-8 text-gray-400"
                    />
